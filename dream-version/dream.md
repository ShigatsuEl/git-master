...
1. `git blame (파일)`명령어를 사용하여 그 파일을 누가 언제 무엇을 수정했는지 다 확인할 수 있다. 개인적으로는 blame보다는 UI를 사용해서 확인하는 것이 빠를 것 같다.
2. VSC Extension에서 `gitLens`라는 플러그인이 있는데 설치 후 아이콘을 통해 간편하게 git blame을 VSC를 통해 확인할 수 있다.(이 방법이 그나마 나은 듯함)

### 문제의 원인을 빠르게 찾는 방법

눈에 보이는 디버그라면 쉽게 문제를 해결할 수 있지만 만약에 코드상으로는 오류가 없어서 문제를 찾기 어려운 상황이라면 `git bisect`를 사용해 아주 유용하게 해결할 수 있습니다.<br>
![07-1](./Subject7/07-1.PNG)
git bisect는 이진탐색으로 문제가 어디서 발생했는지 쉽게 알게해줍니다.<br>
예를들어 최근들어 없던 오류가 발생했다면 문제가 없는 커밋으로 checkout명령어를 이용해 이동한 후에,<br>
`git bisect start`명령어로 시작하고 `git bisect good`명령어를 사용해 이 커밋까지는 오류가 없고 good이라고 정해줍니다.<br>
